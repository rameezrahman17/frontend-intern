...
import { BookOpen, Flame, BarChart3, Settings, Bell, Search, Trophy, Zap, Mail, User } from 'lucide-react';
...
export default function DashboardShell({ courses, profile, activityLogs, leaderboard }: DashboardShellProps) {
  ...
  const renderContent = () => {
    switch (activeTab) {
        ...
        return (
          <div className="space-y-6 max-w-7xl mx-auto w-full pb-20">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {courses.map((course) => (
                <GlowCard key={course.id} className="p-6 flex flex-col justify-between min-h-[220px]" glowColor="rgba(6, 182, 212, 0.15)">
                  <div className="flex justify-between items-start">
                    <div className="p-3 rounded-2xl bg-zinc-900 border border-zinc-800 text-cyan-400">
                      <BookOpen size={24} />
                    </div>
                    <span className="text-sm font-extrabold text-cyan-400">{course.progress}% Completed</span>
                  </div>
                  <div className="my-6">
                    <h3 className="text-lg font-bold text-zinc-100">{course.title}</h3>
                    <p className="text-xs text-zinc-500 mt-2 font-medium">Progress tracked with Supabase PostgreSQL</p>
                  </div>
                  <div className="w-full bg-zinc-900 border border-zinc-800/50 h-2.5 rounded-full overflow-hidden">
                    <div className="bg-cyan-500 h-full rounded-full" style={{ width: `${course.progress}%` }} />
                  </div>
                </GlowCard>
              ))}
            </div>
        ...
        return (
          <div className="space-y-6 max-w-4xl mx-auto w-full pb-20">
            <GlowCard className="p-8 text-center" glowColor="rgba(249, 115, 22, 0.15)">
              ...
              <h2 className="text-xl font-extrabold text-zinc-100">You are on a {profile.streak_days}-Day Streak!</h2>
              <p className="text-sm text-zinc-400 mt-2 font-medium max-w-sm mx-auto">
                Learning every day builds habits that stick. You are in the top 5% of active learners this week.
              </p>
              <div className="mt-6 flex items-center justify-center gap-2 text-cyan-400 font-bold text-lg">
                ...
                <span>{(profile.total_xp ?? 0).toLocaleString()} Total XP</span>
              </div>
            </GlowCard>
          </div>
        );
        ...
        return (
          <div className="space-y-6 max-w-4xl mx-auto w-full pb-20">
            <GlowCard className="p-8 text-center" glowColor="rgba(139, 92, 246, 0.15)">
              ...
              <p className="text-sm text-zinc-400 mt-2 font-medium max-w-sm mx-auto">
                Detailed breakdowns of study duration, progress curves, and subject masteries will be visible here.
              </p>
            </GlowCard>
          </div>
        );
      ...
      case 'settings':
        return (
          <div className="space-y-6 max-w-4xl mx-auto w-full pb-20">
            {/* Profile Card — email visible here */}
            <GlowCard className="p-8" glowColor="rgba(236, 72, 153, 0.15)">
              <div className="flex items-center gap-5 mb-6">
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-violet-500 to-fuchsia-500 flex items-center justify-center text-xl font-bold text-white shadow-inner select-none">
            ...
            <GlowCard className="p-8 text-center" glowColor="rgba(236, 72, 153, 0.10)">
              ...
              <p className="text-sm text-zinc-400 mt-2 font-medium max-w-sm mx-auto">
                Manage credentials, subscription models, theme templates, and database sync configurations.
              </p>
            </GlowCard>
          </div>
        );
  ...
  const getHeaderTitle = () => {
    switch (activeTab) {
      ...
      case 'courses':     return 'My Learning Pathways';
      case 'progress':    return 'Streak Dashboard';
      case 'analytics':   return 'Performance Insights';
      case 'leaderboard': return 'Leaderboard';
      case 'settings':    return 'Profile & Settings';
      default:            return 'NexLearn';
    }
  };
    ...
    <div className="flex min-h-screen bg-zinc-950 text-zinc-50 font-sans selection:bg-cyan-500/30 selection:text-cyan-200">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-20 border-b border-zinc-900/60 px-4 md:px-8 flex items-center justify-between sticky top-0 bg-zinc-950/85 backdrop-blur-md z-30">
          {/* Left: spacer for hamburger on mobile, title on all */}
          <div className="flex items-center gap-3">
            {/* Spacer so title doesn't overlap hamburger button on mobile */}
      ...
      {/* Mobile nav (hamburger + bottom tabs): mobile only */}
      <MobileNav activeTab={activeTab} setActiveTab={setActiveTab} profile={profile} />
    </div>
  );
}
